...
// Заполняется при загрузке страницы и используется для проверок при сохранении случая.
let diagnosesMap = {};

// Группы диагнозов, относящиеся к ИППП.
// Предупреждение показывается если пациент младше 18 лет и диагноз из этих групп.
// Группы-исключения (Микроспория, Микозы, Чесотка) в список НЕ входят.
...
// Когда пользователь нажимает "Продолжить" или "Исправить" — вызывается эта функция.
...
// Использование: const confirmed = await showWarningModal('Текст предупреждения');
function showWarningModal(message) {
    return new Promise((resolve) => {
        // Сохраняем resolve чтобы вызвать его при нажатии кнопки
...
// Вызывается при нажатии кнопок модалки ("Продолжить" или "Исправить")
// и при нажатии на крестик закрытия.
// choice: true = продолжить, false = исправить/отмена
function warningModalResolve(choice) {
    closeModal('warningModal');

    // Вызываем resolve Promise с выбором пользователя
...
// из которого мы берём название группы для проверки ИППП.
async function loadDiagnosesWithGroups() {
    const select = document.getElementById('caseDiagnosis');
    if (!select) return;
    ...
    try {
        ...
        diagnoses.forEach(diagnosis => {
            ...
            // Сохраняем в карту для последующих проверок
    ...
    document.getElementById('patientInfoCard').innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            ...
            <div style="display: flex; gap: 8px;">
                <button class="btn btn-secondary btn-sm" onclick="openEditPatientModal()">Редактировать</button>
                <button class="btn btn-danger btn-sm" onclick="confirmDeletePatient()">Удалить</button>
            </div>
        </div>
    `;
}
        ...
        <div class="case-card" data-id="${c.id}">
            <div class="case-header">
                ...
                <div class="case-actions">
                    <button class="btn btn-secondary btn-sm" onclick="openEditCaseModal(${c.id})">Изменить</button>
                    <button class="btn btn-danger btn-sm" onclick="confirmDeleteCase(${c.id})">Удалить</button>
                </div>
            </div>
...
async function deletePatient() {
    try {
        ...
        setTimeout(() => { window.location.href = 'patient-search.html'; }, 1000);
    } catch (error) {
...
function openAddCaseModal() {
    ...
    // Подставляем соц. данные из последнего случая чтобы не вводить повторно
...
async function saveCase() {
    ...
    // Показываем внутреннюю модалку вместо браузерного confirm()
    if (patientData.birthDate) {
        const birthYear = new Date(patientData.birthDate).getFullYear();
        const currentYear = new Date().getFullYear();

        if (birthYear === currentYear) {
            // await ждёт пока пользователь нажмёт кнопку в модалке
    ...
    const selectedDiagnosisId = document.getElementById('caseDiagnosis').value;
    const selectedGroupName = diagnosesMap[selectedDiagnosisId]?.groupName || '';
    const isIppiDiagnosis = IPPI_GROUPS.includes(selectedGroupName);

    if (typeof age === 'number' && age < 18 && isIppiDiagnosis) {
        const confirmed = await showWarningModal(
...
function confirmDeleteCase(caseId) {
    document.getElementById('deleteConfirmText').textContent = 'Удалить случай заболевания?';
