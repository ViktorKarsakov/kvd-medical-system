...
function renderUsersTable(users) {
 ...
 const toggleBtn = user.enabled
     ? `<button class="btn btn-small btn-danger" onclick="toggleUser(${user.id}, false)" title="Заблокировать">Заблокировать</button>`
     : `<button class="btn btn-small btn-success" onclick="toggleUser(${user.id}, true)" title="Разблокировать">Разблокировать</button>`;

     const resetBtn = `<button class="btn btn-small btn-secondary" onclick="openResetModal(${user.id}, '${user.username}')" title="Сбросить пароль">Сброс пароля</button>`;
